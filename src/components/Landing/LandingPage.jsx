...
import { ShoppingCart, Sparkles, ArrowRight, ShieldCheck, PieChart, MessageSquare, Layers, Star, ChevronRight, Mail, User as UserIcon, Check } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans">
          ...
          <div className="flex items-center gap-3">
            ...
            {onOpenCompare && (
              <button
                ...
                className="px-3.5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs border border-slate-200/80 flex items-center gap-2 transition-all cursor-pointer"
              >
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div 
            onClick={onOpenCompare}
            className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3 hover:border-indigo-400 transition-all duration-300 hover:scale-105 hover:-translate-y-1 cursor-pointer group"
          >
            <div className="w-10 h-10 rounded-xl bg-purple-100 text-purple-600 flex items-center justify-center group-hover:bg-indigo-600 group-hover:text-white transition-colors">
              <Layers className="w-5 h-5" />
            </div>
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-slate-900">Compare Matrix</h3>
              <span className="text-[10px] font-bold text-indigo-600 group-hover:underline">Open Tray →</span>
            </div>
            <p className="text-xs text-slate-600 leading-relaxed">
                ...
                <div className="h-44 rounded-xl overflow-hidden bg-slate-100 relative">
                  ...
                  {onToggleCompare && (
                    <button
                      ...
                      title={comparedIds.includes(product.id) ? "Remove from Compare" : "Add to Compare"}
                      className={`absolute top-2 right-2 p-2 rounded-xl text-xs font-semibold backdrop-blur-md border transition-all duration-200 cursor-pointer ${
